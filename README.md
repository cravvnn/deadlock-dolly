...
**Current source: 0.3.7 alpha.** The portable Windows build opens through
...
- Experimental playback smoothing with Off, Light, Balanced and Strong choices.
...
In 0.3.7, **Smoothing** under Shot playback starts on **Balanced**. It smooths
the shared camera timeline, adding about 80 ms of delay during steady movement.
The choice lasts for the editor session; **Off** retains 0.3.6 playback.
Paused-camera controls are unchanged, and the 0.3.6 final-key fix is included.
In-game improvement still needs verification. See the
[smoothing comparison](docs/USER_GUIDE.md#experimental-playback-smoothing)
