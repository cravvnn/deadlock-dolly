...
class DemoPacketTests(unittest.TestCase):
    ...
    def test_sparse_packets_resolve_to_actual_record_not_arbitrary_tick_tolerance(self):
        for compressed in (False, True):
            ...
            self.assertEqual(index.preceding(48393), 48391)
            self.assertEqual(index.preceding(22631), 22631)
            self.assertEqual(index.preceding(48398), 48397)
            self.assertIsNone(index.preceding(0))
