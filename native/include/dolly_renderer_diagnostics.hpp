...
constexpr std::uint32_t kRendererDiagnosticsAbi=1;
constexpr char kRendererDiagnosticsHash[]="386bdc4adfc8b8a0db67520b98b391f872a214e07077cc17a02f10bf94e3b2d8";
constexpr std::uint32_t kRendererDiagnosticsImageSize=0x4d6000;
enum class RendererProbeState : std::uint32_t {Waiting=0,Supported=1,Unsupported=2,Unreadable=3,Racing=4};
...
struct RendererDiagnostics {
 ...
 std::uint64_t present_calls,panel_frames,init_attempts,init_successes,release_calls,resize_calls;
 std::uint32_t pending_count,capacity,allocated_slots,head_index,tail_index;
 std::uint32_t retirement_frame,execution_frame,head_buffer_frame,tail_buffer_frame,image_size;
 char renderer_hash[65];unsigned char hash_padding[7];
 char message[192];unsigned char padding[112];
};
#pragma pack(pop)
static_assert(sizeof(RendererDiagnostics)==512,"Optional graphics diagnostic layout");
static_assert(offsetof(RendererDiagnostics,pending_count)==96,"Optional graphics counter offset");
...
static_assert(kRendererDiagnosticsOffset>=kControlBytes+sizeof(Status),"No overlap with camera status");
static_assert(kRendererDiagnosticsOffset+sizeof(RendererDiagnostics)<=kControlBytes+2048,"No overlap with editor status");
