...
struct EditorConfig {
 char magic[8];
 std::uint32_t sequence,abi,enabled,owner,owner_sequence,selected_camera,camera_count,ack_event,flags,reserved;
...
struct EditorStatus {
 char magic[8];
 std::uint32_t sequence,abi,owner,flags,selected_camera,camera_count,last_event,dropped_events;
 double speed;
...
struct EditorSnapshot {
 ...
 bool enabled=false,focused=false,manual_active=false,ready=false,paused=false,overlay_available=false,input_available=false;
