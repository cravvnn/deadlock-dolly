...
int main() {
    ...
    require(near(mouseA[3], 1.6) && near(mouseA[4], -.8));
    require(near(mouseA[3], mouseB[3]) && near(mouseA[4], mouseB[4]));
    CameraPose inverted = start;
    integrate_flight(inverted, input, .01, 400, .08, true);
    require(near(inverted[3], -1.6));
