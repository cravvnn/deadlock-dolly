...
RawDataFn gRawData=nullptr;RawBufferFn gRawBuffer=nullptr;
SetCursorPosFn gSetCursorPos=nullptr;ClipCursorFn gClipCursor=nullptr;
std::atomic_flag gCursorLock=ATOMIC_FLAG_INIT;
bool key_down(unsigned vk) noexcept {return vk<256&&gKeys[vk].load(std::memory_order_relaxed)&&!gBlocked[vk].load(std::memory_order_relaxed);}
bool focused() noexcept {auto window=gWindow.load();return window&&GetForegroundWindow()==window;}
bool configured() noexcept {auto c=std::atomic_load(&gConfig);return c&&c->enabled&&gConnected.load()&&gOverlay.load()&&gInput.load();}
...
void cursor_mode(EditorOwner owner) noexcept {
 ...
 if(window&&focused()&&configured()&&owner==EditorOwner::Flight&&(gViewFlags.load()&1)){
  RECT r{};if(GetClientRect(window,&r)){POINT points[2]={{r.left,r.top},{r.right,r.bottom}};MapWindowPoints(window,nullptr,points,2);r={points[0].x,points[0].y,points[1].x,points[1].y};if(gClipCursor)gClipCursor(&r);}
 }else if(gClipCursor)gClipCursor(nullptr);
...
bool binding_down(const EditorBinding& b,bool movement=false) noexcept {
 if(!b.vk||!key_down(b.vk))return false;
...
void process_raw(RAWINPUT* raw,bool suppress) noexcept {
 if(!raw)return;
 if(raw->header.dwType==RIM_TYPEMOUSE&&raw->header.dwSize>=sizeof(RAWINPUTHEADER)+sizeof(RAWMOUSE)){
  auto& m=raw->data.mouse;
  if(configured()&&focused()){
   if(gOwner.load()==EditorOwner::Flight&&!(m.usFlags&MOUSE_MOVE_ABSOLUTE)&&(gViewFlags.load()&4)){
    ...
    gMouseX.fetch_add(clamp(m.lLastX));gMouseY.fetch_add(clamp(m.lLastY));
   }
   const USHORT downs[]={RI_MOUSE_LEFT_BUTTON_DOWN,RI_MOUSE_RIGHT_BUTTON_DOWN,RI_MOUSE_MIDDLE_BUTTON_DOWN,RI_MOUSE_BUTTON_4_DOWN,RI_MOUSE_BUTTON_5_DOWN};
   ...
   for(int i=0;i<5;++i){
    if(m.usButtonFlags&downs[i]){bool reserved=reserved_input(keys[i]);if(gOwner.load()==EditorOwner::Panel)overlay_raw_mouse_button(i,true);key_event(keys[i],true);if(reserved)m.usButtonFlags&=~downs[i];}
    if(m.usButtonFlags&ups[i]){bool reserved=reserved_input(keys[i]);if(gOwner.load()==EditorOwner::Panel)overlay_raw_mouse_button(i,false);key_event(keys[i],false);if(reserved)m.usButtonFlags&=~ups[i];}
   }
   if(gOwner.load()==EditorOwner::Panel&&gNoLegacyMouse.load()){float wheel=float(static_cast<SHORT>(m.usButtonData))/WHEEL_DELTA;if(m.usButtonFlags&RI_MOUSE_WHEEL)overlay_raw_mouse_wheel(wheel,0);if(m.usButtonFlags&RI_MOUSE_HWHEEL)overlay_raw_mouse_wheel(0,wheel);}
  }
  if(suppress){m.lLastX=0;m.lLastY=0;m.usButtonFlags=0;m.usButtonData=0;m.ulRawButtons=0;}
 } else if(raw->header.dwType==RIM_TYPEKEYBOARD&&raw->header.dwSize>=sizeof(RAWINPUTHEADER)+sizeof(RAWKEYBOARD)){
  auto& k=raw->data.keyboard;
  bool reserved=reserved_input(k.VKey);
  if(configured()&&focused()){if(gOwner.load()==EditorOwner::Panel)overlay_raw_key(k.VKey,k.MakeCode,!(k.Flags&RI_KEY_BREAK),(k.Flags&RI_KEY_E0)!=0);key_event(k.VKey,!(k.Flags&RI_KEY_BREAK));}
  if(suppress||reserved){k.MakeCode=0;k.VKey=0;k.Flags=RI_KEY_BREAK;k.Message=WM_KEYUP;}
 }
}
UINT WINAPI raw_data_hook(HRAWINPUT handle,UINT command,LPVOID data,PUINT size,UINT header) {
 ...
 if(command==RID_INPUT&&data&&header==sizeof(RAWINPUTHEADER)&&result!=UINT(-1)&&result>=sizeof(RAWINPUTHEADER)){
  auto raw=static_cast<RAWINPUT*>(data);if(raw->header.dwSize<=result)process_raw(raw,owns_input());
 }
 return result;
}
UINT WINAPI raw_buffer_hook(PRAWINPUT data,PUINT size,UINT header) {
 UINT capacity=size?*size:0;UINT result=gRawBuffer(data,size,header);
 if(data&&header==sizeof(RAWINPUTHEADER)&&result!=UINT(-1)){
  auto begin=reinterpret_cast<unsigned char*>(data);auto end=begin+capacity;
  for(UINT i=0;i<result;++i){
   if(begin+sizeof(RAWINPUTHEADER)>end)break;
   auto raw=reinterpret_cast<RAWINPUT*>(begin);if(raw->header.dwSize<sizeof(RAWINPUTHEADER)||raw->header.dwSize>std::size_t(end-begin))break;
   process_raw(raw,owns_input());
   auto next=reinterpret_cast<std::uintptr_t>(begin)+raw->header.dwSize;
   begin=reinterpret_cast<unsigned char*>((next+sizeof(std::uintptr_t)-1)&~(sizeof(std::uintptr_t)-1));
  }
 }
 return result;
}
BOOL WINAPI set_cursor_pos_hook(int x,int y){if(owns_input()&&gOwner.load()==EditorOwner::Panel)return TRUE;return gSetCursorPos(x,y);}
BOOL WINAPI clip_cursor_hook(const RECT* rect){if(owns_input()&&gOwner.load()==EditorOwner::Panel)return gClipCursor(nullptr);return gClipCursor(rect);}
} // namespace
...
bool editor_enqueue(EditorAction action,double value) noexcept {
 ...
 if(action==EditorAction::SetSpeed)gSpeed=value;
 auto sequence=++gLastEvent;EditorEvent& event=gEvents[(sequence-1)%kEditorEventCount];event={};event.sequence=sequence;event.action=std::uint32_t(action);event.value=value;
 for(unsigned i=0;i<7;++i)event.pose[i]=state.pose[i];event.tick=state.tick;event.paused=state.paused;
 gEventLock.clear(std::memory_order_release);return true;
}
...
bool editor_install_input_hooks() noexcept {
 ...
 Target targets[]={{"GetRawInputData",reinterpret_cast<void*>(raw_data_hook),reinterpret_cast<void**>(&gRawData)},{"GetRawInputBuffer",reinterpret_cast<void*>(raw_buffer_hook),reinterpret_cast<void**>(&gRawBuffer)},{"SetCursorPos",reinterpret_cast<void*>(set_cursor_pos_hook),reinterpret_cast<void**>(&gSetCursorPos)},{"ClipCursor",reinterpret_cast<void*>(clip_cursor_hook),reinterpret_cast<void**>(&gClipCursor)}};
 void* addresses[4]{};int created=0,enabled=0;
 for(auto& t:targets){auto address=reinterpret_cast<void*>(GetProcAddress(user,t.name));if(!address||MH_CreateHook(address,t.hook,t.original)!=MH_OK)break;addresses[created++]=address;}
 if(created==4)for(;enabled<created;++enabled)if(MH_EnableHook(addresses[enabled])!=MH_OK)break;
...
void editor_integrate_flight(CameraPose& pose,double dt) noexcept {
 if(!owns_input()||gOwner.load()!=EditorOwner::Flight||!(gViewFlags.load()&2)){gMouseX=0;gMouseY=0;return;}
 auto c=std::atomic_load(&gConfig);if(!c)return;
 bool command_held=false;for(unsigned i=0;i<12;++i)command_held|=binding_down(c->bindings[i]);
 FlightInput in{};
 if(!command_held){
  auto on=[&](unsigned n){return binding_down(c->bindings[n],true)?1.0:0.0;};
  in.forward=on(12)-on(13);in.right=on(15)-on(14);in.up=on(16)-on(17);
  in.yaw=on(20)-on(21);in.pitch=on(23)-on(22);in.roll=on(25)-on(24);
 }
 in.mouse_x=std::clamp<long>(gMouseX.exchange(0),-2000,2000);in.mouse_y=std::clamp<long>(gMouseY.exchange(0),-2000,2000);
 double speed=gSpeed.load();if(binding_down(c->bindings[18],true))speed*=4;if(binding_down(c->bindings[19],true))speed*=.2;
...
bool editor_window_message(HWND window,UINT message,WPARAM wparam,LPARAM lparam,LRESULT& result) noexcept {
 ...
 if(message==WM_INPUT&&before){
  RAWINPUT raw{};UINT bytes=sizeof(raw);UINT got=gRawData(reinterpret_cast<HRAWINPUT>(lparam),RID_INPUT,&raw,&bytes,sizeof(RAWINPUTHEADER));
  if(got!=UINT(-1)&&got>=sizeof(RAWINPUTHEADER)&&raw.header.dwSize<=got)process_raw(&raw,false);
  // DefWindowProc is required to release foreground RAWINPUT storage.
  result=DefWindowProcW(window,message,wparam,lparam);return true;
 }
 if((message==WM_SYSKEYDOWN||message==WM_SYSKEYUP)&&(wparam==VK_F4||wparam==VK_TAB))return false;
 if(message==WM_KEYDOWN||message==WM_SYSKEYDOWN||message==WM_KEYUP||message==WM_SYSKEYUP){
  ...
  bool down=message==WM_KEYDOWN||message==WM_SYSKEYDOWN;key_event(unsigned(wparam),down);
  if(reserved)return true;
  if(before)return true;
 }
 unsigned mousekey=0;bool down=false;
 switch(message){
  case WM_LBUTTONDOWN:mousekey=VK_LBUTTON;down=true;break;case WM_LBUTTONUP:mousekey=VK_LBUTTON;break;
  case WM_RBUTTONDOWN:mousekey=VK_RBUTTON;down=true;break;case WM_RBUTTONUP:mousekey=VK_RBUTTON;break;
  case WM_MBUTTONDOWN:mousekey=VK_MBUTTON;down=true;break;case WM_MBUTTONUP:mousekey=VK_MBUTTON;break;
  case WM_XBUTTONDOWN:mousekey=HIWORD(wparam)==XBUTTON1?VK_XBUTTON1:VK_XBUTTON2;down=true;break;
  case WM_XBUTTONUP:mousekey=HIWORD(wparam)==XBUTTON1?VK_XBUTTON1:VK_XBUTTON2;break;
 }
...
void editor_worker_tick(unsigned char* memory,bool connected) noexcept {
 ...
 static std::uint32_t accepted=~0u,owner_sequence=~0u;static double configured_speed=-1;
 if(memory&&connected){
  auto src=memory+kEditorConfigOffset;auto seq=reinterpret_cast<volatile LONG*>(src+8);
  LONG before=InterlockedCompareExchange(seq,0,0);
  if(!(before&1)&&std::uint32_t(before)!=accepted){
   EditorConfig c{};std::memcpy(&c,src,sizeof(c));MemoryBarrier();LONG after=InterlockedCompareExchange(seq,0,0);
   bool okay=before==after&&!(after&1)&&std::memcmp(c.magic,"DLYEDIT1",8)==0&&c.abi==kEditorAbi&&c.enabled<=1&&c.owner<=4&&c.flags<=1&&std::isfinite(c.speed)&&c.speed>=1&&c.speed<=10000&&std::isfinite(c.sensitivity)&&c.sensitivity>=.001&&c.sensitivity<=10&&std::isfinite(c.duration)&&c.duration>=0&&std::isfinite(c.playhead)&&c.playhead>=0&&c.camera_count<=4096&&c.playback_flags<=3&&std::memchr(c.shot_name,0,sizeof(c.shot_name))&&std::memchr(c.message,0,sizeof(c.message));
   for(auto& b:c.bindings)okay=okay&&b.vk<256&&b.modifiers<8&&b.vk!=VK_F7;
   if(okay){
    std::shared_ptr<const EditorConfig> next;try{next=std::make_shared<const EditorConfig>(c);}catch(...){gConnected=false;gOwner=EditorOwner::Disabled;cursor_mode(EditorOwner::Disabled);return;}
    std::atomic_store(&gConfig,next);accepted=std::uint32_t(after);
    if(c.speed!=configured_speed){gSpeed=c.speed;configured_speed=c.speed;}
    if(c.owner_sequence!=owner_sequence){owner_sequence=c.owner_sequence;editor_set_owner(EditorOwner(c.owner));}
    if(!c.enabled)editor_set_owner(EditorOwner::Disabled);
   }
  }
 }
 ...
 if(tick_now-last_registration_check>1000){
  RAWINPUTDEVICE devices[64]{};UINT count=64;UINT found=GetRegisteredRawInputDevices(devices,&count,sizeof(RAWINPUTDEVICE));bool no_legacy=false;
  if(found!=UINT(-1))for(UINT i=0;i<found;++i)if(devices[i].usUsagePage==1&&devices[i].usUsage==2)no_legacy=(devices[i].dwFlags&RIDEV_NOLEGACY)==RIDEV_NOLEGACY;
  gNoLegacyMouse=no_legacy;last_registration_check=tick_now;
 }
 if(!memory)return;
 ...
 else if(state.dropped_events)std::snprintf(status.message,sizeof(status.message),"An editor action could not be queued. Wait for the current operation, then retry (%u dropped).",state.dropped_events);
 for(unsigned i=0;i<7;++i)status.applied_pose[i]=state.pose[i];
 if(gEventLock.test_and_set(std::memory_order_acquire))return;
 status.last_event=gLastEvent;std::memcpy(status.events,gEvents,sizeof(gEvents));gEventLock.clear(std::memory_order_release);
 auto out=memory+kEditorStatusOffset;auto sequence=reinterpret_cast<volatile LONG*>(out+8);LONG old=InterlockedCompareExchange(sequence,0,0);LONG even=(old&1)?old+1:old;
 InterlockedExchange(sequence,even+1);std::memcpy(out,&status,8);std::memcpy(out+12,reinterpret_cast<unsigned char*>(&status)+12,sizeof(status)-12);MemoryBarrier();InterlockedExchange(sequence,even+2);
}
} // namespace dolly
