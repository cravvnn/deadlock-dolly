...
double number(const unsigned char* p) noexcept {
    std::uint64_t bits = 0;
    for (unsigned i = 0; i < 8; ++i) bits |= std::uint64_t(p[i]) << (8 * i);
    double result;
    std::memcpy(&result, &bits, sizeof(result));
    return result;
}
bool valid_pose(const CameraPose& pose) noexcept {
    for (double value : pose) if (!std::isfinite(value) || std::abs(value) > 1e8) return false;
...
struct Projection {
    ...
    bool setup(const VisualizationView& view) noexcept {
        if (!valid_pose(view.pose) || !std::isfinite(view.horizontal_fov) ||
            view.horizontal_fov <= 1 || view.horizontal_fov >= 179 ||
            !std::isfinite(view.width) || !std::isfinite(view.height) ||
            view.width <= 0 || view.height <= 0 || view.width > 65536 || view.height > 65536 ||
            !std::isfinite(view.near_plane) || view.near_plane < .001 || view.near_plane > 10000) return false;
        ...
        width=view.width; height=view.height; near_plane=view.near_plane;
    ...
    std::array<double,5> planes(const Vec3& p) const noexcept {
        return {p[2]-near_plane,p[2]*tan_x+p[0],p[2]*tan_x-p[0],p[2]*tan_y+p[1],p[2]*tan_y-p[1]};
    }
    ...
    bool point(const Vec3& world, VisualizationPoint& out) const noexcept {
        ...
        for (double d:planes(p)) if (d < 0 || !std::isfinite(d)) return false;
        out=screen(p); return true;
    }
    bool line(const Vec3& world_a, const Vec3& world_b,
              VisualizationPoint& out_a, VisualizationPoint& out_b) const noexcept {
        if (!finite(world_a) || !finite(world_b)) return false;
        ...
        for (std::size_t i=0;i<pa.size();++i) {
            if (!std::isfinite(pa[i]) || !std::isfinite(pb[i])) return false;
            if (pa[i]<0 && pb[i]<0) return false;
            if (pa[i]<0) enter=std::max(enter,pa[i]/(pa[i]-pb[i]));
            else if (pb[i]<0) leave=std::min(leave,pa[i]/(pa[i]-pb[i]));
        ...
        if (clipped_a[2]<=0 || clipped_b[2]<=0) return false;
        out_a=screen(clipped_a); out_b=screen(clipped_b); return true;
    }
};
Vec3 position(const CameraPose& p) noexcept { return {p[0],p[1],p[2]}; }
} // namespace
...
bool VisualizationPath::load(const void* data, std::size_t bytes, std::string& error) {
    ...
    if (std::memcmp(raw,"DLYVIS01",8)!=0) return fail("Viewer packet magic does not match");
    const auto sequence=u32(raw+8),abi=u32(raw+12),enabled=u32(raw+16),selected=u32(raw+20);
    const auto count=u32(raw+24),path_bytes=u32(raw+28),samples=u32(raw+32),markers=u32(raw+36);
    ...
    for (std::size_t i=40;i<kVisualizationHeaderBytes;++i)
        if (raw[i]) return fail("Viewer packet has unknown reserved fields");
    if (!enabled) {
    ...
    NativePath path;
    if (!path.load(blob,path_bytes,error)) return false;
    if (u32(blob+12)+1!=count) return fail("Viewer timestamps do not match the camera path");
    for (std::uint32_t i=0;i<count;++i) {
        const double time=number(times+i*8);
        const double expected=i+1==count ? number(blob+40) :
            number(blob+(count==1 ? 32 : NativePath::header_bytes+i*NativePath::segment_bytes));
    ...
    candidate.enabled_=true; candidate.selected_camera_=selected; candidate.camera_count_=count;
    try {
        const std::size_t segments=count-1;
        const std::size_t edges=segments ? std::min<std::size_t>(samples-1,segments*32) : 0;
        candidate.points_.reserve(edges+1); candidate.breaks_.reserve(edges+1);
        CameraPose pose{};
        if (!path.evaluate(number(times),pose) || !valid_pose(pose))
            return fail("Viewer path produced an unsupported camera pose");
        candidate.points_.push_back(pose); candidate.breaks_.push_back(1);
        for (std::size_t i=0;i<segments;++i) {
            ...
            for (std::size_t channel=0;channel<3;++channel) {
                const auto* record=blob+NativePath::header_bytes+i*NativePath::segment_bytes+16+40*channel;
                position_step|=u32(record)==0 && number(record+8)!=number(record+16);
            }
            for (std::size_t j=1;j<=steps;++j) {
                const double time=j==steps ? end : begin+(end-begin)*(double(j)/double(steps));
        ...
        if (visible==count) for (std::uint32_t i=0;i<count;++i) indices.push_back(i);
        else {
            indices.push_back(selected);
            for (std::size_t j=0;j+1<visible;++j) {
                const auto index=std::uint32_t(visible==2 ? 0 : j*(count-1)/(visible-2));
                if (index!=selected) indices.push_back(index);
            }
...
bool project_visualization_line(const VisualizationView& view, const Vec3& a, const Vec3& b,
                                VisualizationPoint& screen_a, VisualizationPoint& screen_b) noexcept {
    Projection projection;
...
bool project_visualization(const VisualizationPath& path, const VisualizationView& view,
    ...
    Projection projection;
    if (!projection.setup(view)) return false;
    const auto append=[&](const Vec3& a,const Vec3& b,VisualizationKind kind) {
        if (out.line_count>=out.lines.size()) return;
        auto& line=out.lines[out.line_count];
        if (projection.line(a,b,line.a,line.b)) {line.kind=kind; ++out.line_count;}
    };
    const auto& points=path.points();
    for (std::size_t i=1;i<points.size();++i)
        if (!path.breaks()[i]) append(position(points[i-1]),position(points[i]),VisualizationKind::Path);
    for (const auto& camera:path.cameras()) {
        ...
        const auto kind=camera.index==path.selected_camera() ? VisualizationKind::SelectedCamera : VisualizationKind::Camera;
        ...
        for (std::size_t i=0;i<corners.size();++i)
            for (std::size_t axis=0;axis<3;++axis)
                corners[i][axis]=center[axis]+24*basis.forward[axis]+12*xs[i]*basis.right[axis]+12/pose[6]*ys[i]*basis.up[axis];
        for (std::size_t i=0;i<corners.size();++i) {
