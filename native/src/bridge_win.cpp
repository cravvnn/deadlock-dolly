...
constexpr unsigned char kSetupBytes[]={0x48,0x8b,0xc4,0x48,0x89,0x58,0x10,0x55,0x56,0x57,0x41,0x54,0x41,0x55,0x41,0x56,0x41,0x57,0x48,0x81,0xec,0xc0,0x08,0x00,0x00,0x0f,0x29,0x70,0xb8,0x4c,0x8b,0xf9};
...
static std::wstring beside_module(const wchar_t* name) {
 auto path=module_path(gModule); auto i=path.find_last_of(L"\\/");
...
static bool development_flags() {
 ...
 for(int i=1;i<count;++i){dev|=_wcsicmp(args[i],L"-dev")==0;insecure|=_wcsicmp(args[i],L"-insecure")==0;}
 LocalFree(args); return dev&&insecure;
}
static bool read_memory(std::uintptr_t from,void* to,std::size_t bytes) noexcept {
 SIZE_T read=0;return from && ReadProcessMemory(GetCurrentProcess(),reinterpret_cast<void*>(from),to,bytes,&read) && read==bytes;
}
...
static std::string hash_file(const std::wstring& path) {
 ...
 unsigned char digest[32]{}; std::string result;
 if(BCryptOpenAlgorithmProvider(&alg,BCRYPT_SHA256_ALGORITHM,nullptr,0)>=0 && BCryptCreateHash(alg,&hash,nullptr,0,nullptr,0,0)>=0){
  std::vector<unsigned char> block(1024*1024);DWORD n=0;bool okay=true;
  for(;;){if(!ReadFile(file,block.data(),DWORD(block.size()),&n,nullptr)){okay=false;break;}if(!n)break;if(BCryptHashData(hash,block.data(),n,0)<0){okay=false;break;}}
  if(okay&&BCryptFinishHash(hash,digest,sizeof(digest),0)>=0){static const char digits[]="0123456789abcdef";for(auto byte:digest){result+=digits[byte>>4];result+=digits[byte&15];}}
 }
 if(hash)BCryptDestroyHash(hash);if(alg)BCryptCloseAlgorithmProvider(alg,0);CloseHandle(file);return result;
}
static void write_status(Status status,bool may_wait=false) noexcept {
 if(!gMemory)return;
 while(gStatusLock.test_and_set(std::memory_order_acquire)){if(!may_wait)return;Sleep(1);}
 auto out=gMemory+kControlBytes;
 auto sequence=reinterpret_cast<volatile LONG*>(out+8);
 LONG old=InterlockedCompareExchange(sequence,0,0);LONG even=(old&1)?old+1:old;
 InterlockedExchange(sequence,even+1);
 std::memcpy(status.magic,kStatusMagic,8);status.abi=kBridgeAbi;status.game_pid=GetCurrentProcessId();status.real_time=now_seconds();
 std::memcpy(out,&status,8);std::memcpy(out+12,reinterpret_cast<unsigned char*>(&status)+12,sizeof(Status)-12);
 MemoryBarrier();InterlockedExchange(sequence,even+2);gStatusLock.clear(std::memory_order_release);
}
static void startup_status(State state,unsigned error,const char* message) noexcept {
 Status status{};status.state=std::uint32_t(state);status.error=error;
 std::snprintf(status.message,sizeof(status.message),"%s",message);write_status(status,true);
}
static bool read_control(ControlHeader& h,std::vector<unsigned char>& payload,std::uint32_t accepted) {
 auto sequence=reinterpret_cast<volatile LONG*>(gMemory+12);
 LONG before=InterlockedCompareExchange(sequence,0,0);if(before&1)return false;
 std::memcpy(&h,gMemory,sizeof(h));
 if(h.command==accepted)return false;
 if(h.payload_bytes>kMaxPayloadBytes)return false;
 payload.resize(h.payload_bytes);if(h.payload_bytes)std::memcpy(payload.data(),gMemory+kPayloadOffset,h.payload_bytes);
 MemoryBarrier();LONG after=InterlockedCompareExchange(sequence,0,0);
 return before==after && !(after&1) && std::memcmp(h.magic,kControlMagic,8)==0 && h.abi==kBridgeAbi;
}
static bool read_config(std::wstring& mapping,DWORD& editor_pid) {
 ...
 char data[1024]{};DWORD n=0;bool okay=ReadFile(file,data,sizeof(data)-1,&n,nullptr)!=0;CloseHandle(file);
 if(!okay||!n)return false;
 char token[65]{};unsigned long pid=0;
 if(std::sscanf(data,"DOLLY_NATIVE_1\n%64s\n%lu",token,&pid)!=2 || std::strlen(token)!=32 || !pid)return false;
 for(const char* s=token;*s;++s)if(!((*s>='0'&&*s<='9')||(*s>='a'&&*s<='f')))return false;
 mapping=L"Local\\DeadlockDollyNative_";for(char c:std::string(token))mapping+=wchar_t(c);
 editor_pid=DWORD(pid);return true;
}
static bool module_matches(HMODULE module,const char* expected,std::uint32_t image_size) {
 if(!module||hash_file(module_path(module))!=expected)return false;
...
static bool read_demo(DemoState& result) noexcept {
 ...
 if(!read_value(gEngine+kDemoGlobal,demo)||!read_value(demo,table)||table!=gEngine+kDemoTable)return false;
 ...
 if(!read_value(gClient+kEngineClient,engine_client)||!read_value(engine_client,engine_table)||engine_table!=gEngine+kEngineTable)return false;
 auto active=reinterpret_cast<bool(__fastcall*)(void*)>(gEngine+0x7bf30);
 if(!active(reinterpret_cast<void*>(engine_client))){result.playing=false;return true;}
 result.paused=reinterpret_cast<bool(__fastcall*)(void*)>(gEngine+0x368d0)(reinterpret_cast<void*>(demo));
 result.seeking=reinterpret_cast<bool(__fastcall*)(void*)>(gEngine+0x2a240)(reinterpret_cast<void*>(demo));
 result.tick=reinterpret_cast<int(__fastcall*)(void*)>(gEngine+0x36910)(reinterpret_cast<void*>(demo));
 auto name=reinterpret_cast<const char*(__fastcall*)(void*)>(gEngine+0x7bf80)(reinterpret_cast<void*>(engine_client));
 ...
 while(used<sizeof(result.name)-1){
  MEMORY_BASIC_INFORMATION region{};auto p=reinterpret_cast<std::uintptr_t>(name)+used;
  if(!VirtualQuery(reinterpret_cast<void*>(p),&region,sizeof(region))||region.State!=MEM_COMMIT)return false;
  auto available=reinterpret_cast<std::uintptr_t>(region.BaseAddress)+region.RegionSize-p;
  auto amount=std::min<std::size_t>({available,sizeof(result.name)-1-used,64});
  if(!amount||!read_memory(p,result.name+used,amount))return false;
  if(std::memchr(result.name+used,0,amount))break;used+=amount;
 }
 if(!std::memchr(result.name,0,sizeof(result.name)-1))return false;
 if(!read_value(gClient+kGlobals,globals))return false;
 float current=0,interval=0;
 if(!read_value(globals+0x30,current)||!read_value(globals+0x54,interval))return false;
 if(!std::isfinite(current)||current<0||!std::isfinite(interval)||interval<.001f||interval>.2f)return false;
 result.time=current;result.interval=interval;return true;
}
static bool same_demo(const char* expected,const char* actual) noexcept {
 auto basename=[](const char* s){const char* b=s;for(const char* p=s;*p;++p)if(*p=='/'||*p=='\\')b=p+1;return b;};
 return *expected && *actual && _stricmp(basename(expected),basename(actual))==0;
}
static bool pose_valid(const CameraPose& p) noexcept {
 for(double v:p)if(!std::isfinite(v)||std::abs(v)>1e8)return false;
 return p[6]>=.25&&p[6]<=8;
}
...
static void on_view(void* self,std::uintptr_t caller) noexcept {
 ...
 struct Exit {bool& flag;~Exit(){flag=false;}} exit{entered};
 ++gHookCalls;
 std::uintptr_t table=0;
 if(caller!=gClient+kCaller||!read_value(reinterpret_cast<std::uintptr_t>(self),table)||table!=gClient+kViewTable)return;
 auto view=reinterpret_cast<std::uintptr_t>(self)+0x10;
 static std::uint32_t seen=0;
 static double phase=0,play_base=0,anchor=0,real_anchor=0,last_game=0,last_real=0,largest_interval=0;
 ...
 Status status{};status.frame_count=++frames;status.hook_calls=gHookCalls.load();
 double now=now_seconds();status.frame_interval_ms=last_real?1000*(now-last_real):0;
 largest_interval=std::max(largest_interval,status.frame_interval_ms);status.max_frame_interval_ms=largest_interval;
 double real_delta=last_real?std::max(0.0,now-last_real):0;last_real=now;
 float original_xyz[3]{},original_angles[3]{},original_fov=0,original_aspect=0;
 unsigned char flags=0;int width=0,height=0;
 bool view_ok=read_memory(view+0x4a0,original_xyz,sizeof(original_xyz))&&read_memory(view+0x4b8,original_angles,sizeof(original_angles))&&read_value(view+0x498,original_fov)&&read_value(view+0x4d8,original_aspect)&&read_value(view+0x555,flags)&&read_value(view+0x430,width)&&read_value(view+0x438,height);
 CameraPose original{};
 for(int i=0;i<3;++i){original[i]=original_xyz[i];original[i+3]=original_angles[i];}
 original[6]=original_aspect;
 for(int i=0;i<7;++i){status.original_pose[i]=original[i];status.applied_pose[i]=original[i];}
 ...
 if(c.command!=seen){
  ...
  if(c.mode==std::uint32_t(Mode::Manual)){
   ...
   play_base=phase;
   real_anchor=now;started=false;completed=false;largest_interval=0;
  }
  else if(c.mode==std::uint32_t(Mode::HoldCurrent)){anchor=demo.time;started=false;completed=false;}
  previous_mode=c.mode;last_game=demo.time;last_tick=demo.tick;
 }
 if(c.mode==std::uint32_t(Mode::Release)){finish(State::Stopped,0,"Native camera released; the game owns the view.");return;}
 if(gWorkerError.load()||now-gHeartbeatTime.load()>2.0){fault=10;finish(State::Fault,fault,"Dolly control connection expired; native camera released.");return;}
 if(fault){finish(State::Fault,fault,"Native camera stopped; restart the shot after checking diagnostics.");return;}
 if(!view_ok||!pose_valid(original)||!std::isfinite(original_fov)||original_fov<=1||original_fov>=179||width<=0||height<=0||(flags&2)){
  fault=11;finish(State::Fault,fault,"This view or projection is unsupported; native camera released.");return;
 }
 ...
 if(c.mode==std::uint32_t(Mode::Hold)||c.mode==std::uint32_t(Mode::HoldCurrent)){
  anchor=demo.time;real_anchor=now;last_game=demo.time;last_tick=demo.tick;
 } else if(!completed){
  if(c.flags&kFrozen){
   if(!demo.paused||demo.tick!=last_tick){fault=14;finish(State::Fault,fault,"The replay moved during frozen native playback.");return;}
   ...
   if(!demo.paused){
    ...
    if(delta < -2*demo.interval || delta>permitted || demo.tick<last_tick || demo.tick-last_tick>128){fault=15;finish(State::Fault,fault,"Replay time jumped during native playback; camera released.");return;}
    started=true;
    phase=std::max(phase,play_base+demo.time-anchor);
   } else if(!started){anchor=demo.time;}
   last_game=demo.time;last_tick=demo.tick;
  }
  if(phase>=command->path->duration()){phase=command->path->duration();completed=true;}
 }
 CameraPose applied{};
 if(!command->path->evaluate(phase,applied)||!pose_valid(applied)){fault=16;finish(State::Fault,fault,"Native path returned an invalid camera.");return;}
 double fov=original_fov;
 if(c.flags&kAspect){
 ...
 if(c.flags&kAspect){float value=float(fov),aspect=float(applied[6]);std::memcpy(reinterpret_cast<void*>(view+0x498),&value,4);std::memcpy(reinterpret_cast<void*>(view+0x4d8),&aspect,4);}
 for(int i=0;i<7;++i)status.applied_pose[i]=applied[i];status.applied_fov=fov;
 finish(completed?State::Completed:(c.mode==std::uint32_t(Mode::Play)?State::Playing:State::Armed),0,
        completed?"Native path finished; holding the final camera for handoff.":(c.mode==std::uint32_t(Mode::Play)?"Native path evaluates during each main-view setup.":"Native camera held; waiting for playback or handoff."));
}
...
static DWORD WINAPI worker(void*) {
 try {
  std::wstring mapping;DWORD editor_pid=0;
  if(!read_config(mapping,editor_pid))return 0;
  gMapping=OpenFileMappingW(FILE_MAP_ALL_ACCESS,FALSE,mapping.c_str());if(!gMapping)return 0;
  gMemory=static_cast<unsigned char*>(MapViewOfFile(gMapping,FILE_MAP_ALL_ACCESS,0,0,kMappingBytes));if(!gMemory)return 0;
  gEditor=OpenProcess(SYNCHRONIZE,FALSE,editor_pid);
  if(!gEditor){startup_status(State::Fault,20,"Could not verify the Dolly editor process.");return 0;}
  startup_status(State::Starting,0,"Waiting for Deadlock client and engine modules.");
  double began=now_seconds();HMODULE client=nullptr,engine=nullptr;
  ...
  if(!GetModuleHandleExW(GET_MODULE_HANDLE_EX_FLAG_FROM_ADDRESS|GET_MODULE_HANDLE_EX_FLAG_PIN,
   ...
   startup_status(State::Fault,25,"Could not keep the native callback resident; no hook was installed.");return 0;
  }
  if(MH_Initialize()!=MH_OK||MH_CreateHook(reinterpret_cast<void*>(gClient+kSetup),reinterpret_cast<void*>(setup_hook),reinterpret_cast<void**>(&gOriginalSetup))!=MH_OK){startup_status(State::Fault,23,"Could not prepare the native view hook.");return 0;}
...
static BOOL CALLBACK start_worker(PINIT_ONCE,PVOID,PVOID*) {
 HANDLE thread=CreateThread(nullptr,0,worker,nullptr,0,nullptr);if(thread)CloseHandle(thread);return TRUE;
}
static BOOL CALLBACK load_unlocker(PINIT_ONCE,PVOID,PVOID*) {
 if(!development_flags())return TRUE;
 auto path=beside_module(L"dolly_cvar_unlocker.dll");
 if(path.empty()||hash_file(path)!=kUnlockerHash)return TRUE;
 HMODULE module=LoadLibraryExW(path.c_str(),nullptr,LOAD_WITH_ALTERED_SEARCH_PATH);
 if(module)gOriginalFactory=reinterpret_cast<Factory>(GetProcAddress(module,"CreateInterface"));
 return TRUE;
}
}
...
extern "C" __declspec(dllexport) void* CreateInterface(const char* name,int* result) {
 try {
  ...
  if(!gOriginalFactory){if(result)*result=1;OutputDebugStringA("Dolly native: development flags or verified cvar unlocker missing.\n");return nullptr;}
  void* original=gOriginalFactory(name,result);
  InitOnceExecuteOnce(&gWorkerOnce,start_worker,nullptr,nullptr);
  return original;
 }catch(...){if(result)*result=1;return nullptr;}
}
BOOL WINAPI DllMain(HINSTANCE module,DWORD reason,LPVOID) {
 if(reason==DLL_PROCESS_ATTACH){gModule=module;QueryPerformanceFrequency(&gFrequency);}
 return TRUE;
}
